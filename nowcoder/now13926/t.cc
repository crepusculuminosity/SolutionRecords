...
struct State {
  ...
  State(int x_, int y_, int vx_, int vy_, int p_, int dist_) {
    x = x_;
    y = y_;
    vx = vx_;
    vy = vy_;
  ...
  bool isValid() {
    return (x >= 0 && y >= 0 && x < R && y < C && vx >= -10 && vy >= -10 &&
            vx <= 10 && vy <= 10 && A[x][y] >= -1);
  }
};
...
int main() {
  ...
  for (int y = 0; y < Q.size(); y++) {
    ...
    for (int i = 0; i < 9; i++) {
      State S(Q[y].x + Q[y].vx + dx[i], Q[y].y + Q[y].vy + dy[i],
              Q[y].vx + dx[i], Q[y].vy + dy[i], Q[y].p, Q[y].dist + 1);
      if (S.isValid()) {
        if (A[S.x][S.y] == Q[y].p + 1)
          S.p++;
        if (visited[S.x][S.y][S.vx + 10][S.vy + 10][S.p])
          continue;
